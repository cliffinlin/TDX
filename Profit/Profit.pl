...
push(@LineArray, "\r");
push(@LineArray, "TRANSACTION_FEE:=2/1000;");
push(@LineArray, "\r");
...
foreach my $Period (@MAPeriod)
{
    ...
    push(@LineArray, "RATIO_$i:=IF(HOLD_$i=1,PERCENT_$i,0);");
    push(@LineArray, "\r");
    
    push(@LineArray, "PRICE_$i:=REF(C,BARSLAST(HOLD_$i<>1)-1);");
    push(@LineArray, "VOLUME_$i:=INTPART(MONEY*RATIO_$i*(1-TRANSACTION_FEE)/PRICE_$i/100)*100;");
    push(@LineArray, "VALUE_$i:=VOLUME_$i*C;");
    push(@LineArray, "\r");
    
    push(@LineArray, "BUY_VOLUME_$i:=IF(BUY_$i=1,VOLUME_$i,0);");
    push(@LineArray, "SELL_VOLUME_$i:=IF(SELL_$i=1,REF(VOLUME_$i,1),0);");
